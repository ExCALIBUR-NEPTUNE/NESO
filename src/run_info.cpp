/*
 * Module for handling run information, such as git revision and state, uuid,
 * and execution device.
 */

#include "run_info.hpp"

RunInfo::RunInfo(sycl::queue &Q, const char *git_revision_in,
                 const char *git_repo_state_in)
    : git_revision(git_revision_in), git_repo_state(git_repo_state_in) {
...
void RunInfo::report_run_info() {
  ...
  std::cout << "NESO running on " << device_name << std::endl;
