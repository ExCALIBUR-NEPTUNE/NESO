...
public:
  ...
  inline void evaluate(ParticleGroupSharedPtr particle_group, Sym<U> sym,
    ...
    const int local_mem_required =
        local_size * sizeof(double) * this->max_nummodes_0;
    if (!local_mem_exists || local_mem_size < (local_mem_required)) {
    ...
    auto event_quad = this->sycl_target->queue
        .submit([&](sycl::handler &cgh) {
          sycl::accessor<double, 1, sycl::access::mode::read_write,
              ...
              local_mem(sycl::range<1>(local_mem_required), cgh);
