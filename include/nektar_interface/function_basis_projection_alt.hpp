...
template <typename T> class FunctionProjectBasis : public BasisEvaluateBase<T> {
  ...
  template <typename U>
  auto
  get_device_data_group(ParticleGroupSharedPtr &particle_group, Sym<U> sym,
  ...
  get_device_data_sub_group(ParticleSubGroupSharedPtr &sub_group, Sym<U> sym,
	...
	auto nrow_max = std::max_element(selection.h_npart_cell,selection.h_npart_cell + selection.ncell);	
  ...
  auto
  get_device_data(GroupType &particle_group, Sym<U> sym,
                  Nektar::LibUtilities::ShapeType const shape_type) { 
  ...
  inline sycl::event project_inner(GroupType &particle_group,
    ...
    Utilities::static_case<Constants::min_nummodes, Constants::max_nummodes>(
        k_nummodes, [&](auto I) {
          event = Shape::algorithm::template project<I, U, Constants::alpha,
                                                     Constants::beta, Shape,typename decltype(device_data)::Filter>(
...
public:
  ...
  project(GroupType &particle_group, Sym<U> sym,
    ...
    if (this->sycl_target->queue.get_device().is_gpu() ||
      ...
      dispatch_to_alg<U, V, Project::ThreadPerDof,GroupType>(particle_group, sym,
                                                   component, global_coeffs);
    } else {
      dispatch_to_alg<U, V, Project::ThreadPerCell,GroupType>(particle_group, sym,
                                                    component, global_coeffs);
    }
