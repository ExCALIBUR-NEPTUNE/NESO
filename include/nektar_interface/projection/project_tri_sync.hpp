...
// emodA entry
template <int nmode> inline int calc_tri_row(int N) {
  double n = double(N + 1);
...
template <int NMODE> int calc_tri_row_rev(int dof) {
...
void tri_sync(double const *eta0, double const *eta1, double const *qoi,
              double *dofs, int npar) {
  ...
  for (int thd = 0; thd < Constants::local_size; ++thd) {
    ...
    while (idx_local < ndof) {
      ...
      for (int k = 0; k < count; ++k) {
        ...
        //(***)
        double correction =
            (idx_local == 1) ? 1.0 : localA[i * Constants::gpu_stride + k];
        temp += correction * localB[idx_local * Constants::gpu_stride + k];
      }
      dofs[idx_local] += temp;
