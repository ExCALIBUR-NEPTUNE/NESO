...
template <> struct ePyramid<ThreadPerDof3D> : public Private::ePyramidBase {
  ...
  static inline auto NESO_ALWAYS_INLINE local_mem_size() {
    if constexpr (dim == 0)
      ...
      return Basis::eModPyrC_len<nmode>() * Constants::gpu_stride;
    else {
  ...
  static void NESO_ALWAYS_INLINE fill_local_mem(T eta0, T eta1, T eta2, T qoi,
    ...
    Basis::eModPyrC<T, nmode, Constants::gpu_stride, alpha, beta>(eta2, local2);
...
template <> struct ePyramid<ThreadPerCell3D> : public Private::ePyramidBase {
  ...
  project_one_particle(const T eta0, const T eta1, const T eta2, const T qoi,
    ...
    Basis::eModPyrC<T, nmode, Constants::cpu_stride, alpha, beta>(eta2, local2);
    int mode = 0;
    NESO_UNROLL_LOOP
    for (int i = 0; i < nmode; ++i) {
