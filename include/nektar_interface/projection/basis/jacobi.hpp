...
inline auto __attribute__((always_inline)) jacobi([[maybe_unused]] T const z) {
  ...
  if constexpr (p == 0)
    ...
    return T(T(0.5) * (T(2.0) * T(alpha + 1) + T(alpha + beta + 2) * (z - T(1.0))));
  else if constexpr (p == 2)
    return T(T(0.125)) *
           (T(T(4.0) * (alpha + T(1.0)) * (alpha + T(2.0))) +
            T(T(4.0) * (alpha + beta + T(3.0)) * (alpha + T(2.0))) * (z - (T(1.0))) +
            T((alpha + beta + T(3.0))) * T((alpha + beta + T(4.0))) * (z - (T(1.0))) *
                (z - (T(1.0))));
  else {
    auto n = p - 1;
    auto pn = jacobi<T, p - 1, alpha, beta>(z);
    auto pnm1 = jacobi<T, p - 2, alpha, beta>(z);
    auto coeff_pnp1 = T(T(2.0) * (n + T(1.0)) // 6
                        * (n + alpha + beta + T(1.0)) * (T(2.0) * n + alpha + beta));
    auto coeff_pn =
        T((T(2.0) * n + alpha + beta + T(1.0)) * (alpha * alpha - beta * beta)) +
        T(pochhammer<3>(2 * (p - 1) + alpha + beta)) * z;
    auto coeff_pnm1 =
        T(-T(2.0) * (n + alpha) * (n + beta) * (T(2.0) * n + alpha + beta + T(2.0)));
