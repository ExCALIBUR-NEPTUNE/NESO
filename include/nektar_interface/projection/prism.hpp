...
public:
  ...
  static inline auto NESO_ALWAYS_INLINE local_mem_size() {
    ...
    if constexpr (dim == 1)
      ...
      return Constants::gpu_stride * Basis::eModB_len<nmode>();
    else
  ...
  static void NESO_ALWAYS_INLINE fill_local_mem(T eta0, T eta1, T eta2, T qoi,
    ...
    Basis::eModB<T, nmode, Constants::gpu_stride, alpha, beta>(eta2, local2);
...
template <> struct ePrism<ThreadPerCell3D> : public Private::ePrismBase {
