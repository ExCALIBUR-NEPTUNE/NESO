...
public:
  ...
  bool static inline project(DeviceData<T, Filter> &data, int componant,
    ...
    // NOTE: If on NVIDIA then AFAICT outer_size < INT_MAX but nrow_max is an
    // int all we can do is check that the ROUND_UP won't overflow but can
    // probably get a tighter bound than this These are hardcoded nvidia numbers
    // - can't find them from sycl
    ...
    //	sycl::free(lut_master,queue);
    auto lut = Shape::template get_lut<nmode>(queue);
    //	auto lut = lut_master;
          ...
          .submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for<>(range, [=](sycl::nd_item<2> idx) {
              ...
              while (idx_local < ndof) {
                T temp(0.0);
                if constexpr (Shape::use_lut) {
                  temp = Shape::template reduce_dof<nmode, T>(
                      lut, idx_local, count, mode0, mode1, stride);
                } else {
                  temp = Shape::template reduce_dof<nmode, T>(
