...
private:
    ...
    inline void generic_copy(sycl::buffer<Complex, 1> &out_b, sycl::buffer<Complex, 1> &in_b){
        Q.submit([&] (sycl::handler& cgh) {
            ...
            cgh.parallel_for<class copy_to_buffer_k>(
                sycl::range<1>{size_t(this->N)},
                [=](cl::sycl::id<1> id) {
                    reinterpret_cast<double(&)[2]>(out_a[id])[0] = reinterpret_cast<const double(&)[2]>(in_a[id])[0];
                    reinterpret_cast<double(&)[2]>(out_a[id])[1] = reinterpret_cast<const double(&)[2]>(in_a[id])[1];
                }
            );
        }).wait();
    }
    ...
    inline void copy_from_buffer(std::vector<Complex> &out_vec, sycl::buffer<Complex, 1> &in_b){
        sycl::buffer<Complex, 1> out_b(out_vec.data(), sycl::range<1>{out_vec.size()});
        generic_copy(out_b, in_b);
    }
