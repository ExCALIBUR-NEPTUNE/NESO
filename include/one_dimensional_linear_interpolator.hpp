...
protected:
  virtual void interpolate(std::vector<double> &x_input,
    ...
    auto event_interpolate = this->m_sycl_target->queue.submit([&](sycl::handler
                                                                       &cgh) {
      auto x_data_sycl =
      ...
      cgh.parallel_for<>(sycl::range<1>(y_output_sycl.size()), [=](sycl::id<1>
                                                                       idx) {
        int k;
        for (int i = 0; i < x_data_sycl.size(); i++) {
          // error handling
          if (x_input_sycl[int(idx)] < x_data_sycl[0] or
              x_input_sycl[int(idx)] > x_data_sycl[x_data_sycl.size() - 1]) {
            std::cerr << "OneDimensionalLinearInterpolator: Input value out of "
