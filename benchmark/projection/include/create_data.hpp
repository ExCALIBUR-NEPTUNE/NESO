...
random_fill_array(double *d_ptr, int size, sycl::queue &q)
{
	...
	q.copy<double>(vec.data(),d_ptr, size).wait_and_throw();
}
							   ...
							   int active_cells = -1)
{
  ...
  if (active_cells < 0 || active_cells == ncell) {
	...
	Q.copy<int>(host_par_per_cell.data(),par_per_cell,ncell).wait();
  	...
  	host_par_per_cell = simplesol_dist(ncell,active_cells,min_per_cell,max_per_cell);
	max_row = *std::max_element(host_par_per_cell.begin(),host_par_per_cell.end());
	Q.copy<int>(host_par_per_cell.data(),par_per_cell,ncell).wait();
  }
  //DOFS 
  double *dofs = sycl::malloc_device<double>(ncell * Shape::template get_ndof<nmode>(), Q);
  assert(dofs);
  all_pointers.push_back((void *)dofs);
  Q.fill(dofs, double{0.0}, ncell * Shape::template get_ndof<nmode>()).wait_and_throw();

  //DOF offsets ndof per cell but scanned
  auto host_offsets = std::vector<int>(ncell,Shape::template get_ndof<nmode>());
  ...
  auto host_data_ptrs = std::vector<double**>(ncell,nullptr);
  for(int i = 0; i < ncell; ++i) {	
		...
		host_data_ptrs[i] = sycl::malloc_device<double*>(Shape::dim,Q);
		assert(host_data_ptrs[i]);
		all_pointers.push_back((void*)host_data_ptrs[i]);
		Q.copy<double*>(par_data,host_data_ptrs[i],Shape::dim).wait_and_throw();
  }
  auto positions = sycl::malloc_device<double**>(ncell,Q);
  assert(positions);
  all_pointers.push_back((void *)positions);
  Q.copy<double**>(host_data_ptrs.data(),positions,ncell).wait_and_throw(); 
   
 
  //Particle value data
  
  auto host_value_ptrs = std::vector<double**>(ncell,nullptr);
  for(int i = 0; i < ncell; ++i) {	
		//allocate value arrays for positions
		double* par_vals[1] = {nullptr};
 		par_vals[0] = sycl::malloc_device<double>(host_par_per_cell[i],Q);
		assert(par_vals);
		...
		Q.copy<double*>(par_vals,host_value_ptrs[i],1).wait_and_throw();
  }
  auto input = sycl::malloc_device<double**>(ncell,Q);
  assert(input);
  all_pointers.push_back((void *)input);
  Q.copy<double**>(host_value_ptrs.data(),input,ncell).wait_and_throw(); 
