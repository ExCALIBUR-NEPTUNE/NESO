...
TEST(AtomicDataReadersTest, CSV) {
  ...
  std::filesystem::path csv_test_file = test_resources_dir / "charge_exchange_h0_h1.csv";

  CSVAtomicDataReader atomic_data = CSVAtomicDataReader(std::string(csv_test_file));
