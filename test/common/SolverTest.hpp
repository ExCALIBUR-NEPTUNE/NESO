...
protected:
  ...
  std::ofstream err_strm, out_strm;
  std::streambuf *orig_stdout_buf;
  std::streambuf *orig_stderr_buf;

  /// Directory paths
  std::filesystem::path test_res_dir;
  std::filesystem::path test_run_dir;
  std::filesystem::path common_test_res_dir;
