...
TEST(ParticleFunctionProjection, DisContFieldScalarExpQuantity) {
  ...
  auto project_run = [&](const int N_total, const int samplex) {
    ...
    copy_to_cstring(
        std::string(
            "test/test_resources/square_triangles_quads_nummodes_2.xml"),
        &argv[1]);
...
TEST(ParticleFunctionProjection, ContFieldScalarExpQuantity) {
  ...
  if (size == 1) {
    ...
    auto project_run = [&](const int N_total, const int samplex) {
      ...
      int argc = 3;
      char *argv[3];
      copy_to_cstring(std::string("test_particle_function_evaluation"),
                      &argv[0]);
      copy_to_cstring(
          std::string(
              "test/test_resources/square_triangles_quads_nummodes_2.xml"),
      ...
      auto cell_id_translation = std::make_shared<CellIDTranslation>(
          sycl_target, A->cell_id_dat, mesh);
      ...
      const auto pl_iter_range =
          A->mpi_rank_dat->get_particle_loop_iter_range();
      const auto pl_stride = A->mpi_rank_dat->get_particle_loop_cell_stride();
      const auto pl_npart_cell =
          A->mpi_rank_dat->get_particle_loop_npart_cell();
      const REAL two_over_sqrt_pi = 1.1283791670955126;
      const REAL reweight =
          pbc.global_extent[0] * pbc.global_extent[1] / ((REAL)N_total);
      ...
      // H5Part h5part("exp.h5part", A, Sym<REAL>("P"),
