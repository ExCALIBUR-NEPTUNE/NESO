#ifndef _MAIN_PROJECTION_H_
#define _MAIN_PROJECTION_H_
...
#include "ParticleSystems/evaluate_project_system.hpp"
#include "hdf5_results_writer.hpp"

using namespace NESO::Benchmark;
...
/**
 *  Benchmarking class to project element types separately.
 */
template <typename T> class BenchmarkProject : public FunctionProjectBasis<T> {
...
public:
  /**
   * Constructor to create instance to project Nektar++ fields.
   *
   ...
   * cells.
   */
  BenchmarkProject(std::shared_ptr<T> field,
                   ParticleMeshInterfaceSharedPtr mesh,
                   CellIDTranslationSharedPtr cell_id_translation)
      : FunctionProjectBasis<T>(field, mesh, cell_id_translation) {}
  ...
  /**
   * Initialise the project method.
   *
   * @param global_coeffs source DOFs which are projected.
   */
  template <typename V> inline void project_init(V &global_coeffs) {
  ...
  /**
   * Project nektar++ function at particle locations.
   *
   ...
   */
  template <typename U>
  inline REAL project_quadrilaterals(ParticleGroupSharedPtr particle_group,
    ...
    if ((!vector_exists) || bypass_generated) {
      FunctionProjectBasis<T>::project_inner(ExpansionLooping::Quadrilateral{},
   ...
   */
  template <typename U>
  inline REAL project_triangles(ParticleGroupSharedPtr particle_group,
                                Sym<U> sym, const int component,
                                bool bypass_generated = false) {
    ...
    if ((!vector_exists) || bypass_generated) {
      FunctionProjectBasis<T>::project_inner(ExpansionLooping::Triangle{},
                                             particle_group, sym, component,
                                             event_stack);
    }
   ...
   */
  template <typename U>
  inline REAL project_hexahedrons(ParticleGroupSharedPtr particle_group,
                                  Sym<U> sym, const int component,
                                  bool bypass_generated = false) {
    ...
    if ((!vector_exists) || bypass_generated) {
      FunctionProjectBasis<T>::project_inner(ExpansionLooping::Hexahedron{},
                                             particle_group, sym, component,
                                             event_stack);
    }
   ...
   */
  template <typename U>
  inline REAL project_prisms(ParticleGroupSharedPtr particle_group, Sym<U> sym,
                             const int component,
                             bool bypass_generated = false) {
    ...
    if ((!vector_exists) || bypass_generated) {
      FunctionProjectBasis<T>::project_inner(ExpansionLooping::Prism{},
                                             particle_group, sym, component,
                                             event_stack);
    }
   ...
   */
  template <typename U>
  inline REAL project_tetrahedrons(ParticleGroupSharedPtr particle_group,
                                   Sym<U> sym, const int component,
                                   bool bypass_generated = false) {
    ...
    if ((!vector_exists) || bypass_generated) {
      FunctionProjectBasis<T>::project_inner(ExpansionLooping::Tetrahedron{},
                                             particle_group, sym, component,
                                             event_stack);
    }
   ...
   */
  template <typename U>
  inline REAL project_pyramids(ParticleGroupSharedPtr particle_group,
                               Sym<U> sym, const int component,
                               bool bypass_generated = false) {
    ...
    if ((!vector_exists) || bypass_generated) {
      FunctionProjectBasis<T>::project_inner(ExpansionLooping::Pyramid{},
                                             particle_group, sym, component,
                                             event_stack);
    }
