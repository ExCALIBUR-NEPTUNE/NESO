...
int main_evaluation(int argc, char *argv[], LibUtilities::SessionReaderSharedPtr session) {
  ...
  session->LoadParameter("num_particles_total", num_particles_total, num_particles_total_default);
  ...
  MPICHK(MPI_Allreduce(&N, &num_particles_check, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD));
  NESOASSERT(num_particles_check == num_particles_total, "Error creating particles");
