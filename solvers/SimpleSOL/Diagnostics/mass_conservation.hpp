...
public:
  ...
  inline double compute_particle_mass() {
    ...
    //////////// SOME DEBUGING TODO REMOVE
    auto k_SD = (*particle_group)[Sym<REAL>("SOURCE_DENSITY")]
                   ->cell_dat.device_ptr();
    this->dh_particle_total_weight.h_buffer.ptr[0] = 0.0;
    sycl_target->queue
        .submit([&](sycl::handler &cgh) {
          cgh.parallel_for<>(
              sycl::range<1>(pl_iter_range), [=](sycl::id<1> idx) {
                ...
                const double contrib = k_SD[cellx][0][layerx];
        ...
        .wait_and_throw();
    this->dh_particle_total_weight.device_to_host();
    const double tmp_weight_sd = this->dh_particle_total_weight.h_buffer.ptr[0];
    //////////// SOME DEBUGING TODO REMOVE
  ...
  inline double compute_total_added_mass() {
    ...
    added_mass = ((double) this->particle_sys->total_num_particles_added) * this->particle_sys->particle_weight;
  ...
  inline void compute(int step) {
    ...
    nprint(step, "mass_particles", mass_particles, "mass_fluid", mass_fluid,
           "mass_initial", this->initial_mass_fluid, "mass_added", mass_added,
