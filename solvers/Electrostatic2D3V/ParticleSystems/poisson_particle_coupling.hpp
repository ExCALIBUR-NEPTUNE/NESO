...
private:
  ...
  std::shared_ptr<FieldEvaluate<DisContField>> field_evaluate;

  SessionFunctionSharedPtr forcing_session_function;
  DisContFieldSharedPtr forcing_function;
  DisContFieldSharedPtr potential_function;
...
public:
  PoissonParticleCoupling(LibUtilities::SessionReaderSharedPtr session,
    ...
    this->forcing_function =
        std::dynamic_pointer_cast<DisContField>(forcing_expansion_explist);
    ...
    this->potential_function =
        std::dynamic_pointer_cast<DisContField>(fields[0]);

    // Create evaluation object to compute the gradient of the potential field
    this->field_evaluate = std::make_shared<FieldEvaluate<DisContField>>(
