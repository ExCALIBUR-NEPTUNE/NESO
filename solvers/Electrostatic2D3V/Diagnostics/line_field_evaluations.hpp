...
/**
 * Evaluate the derivative of the potential field at a set of points that form
 * two lines. One line is in the x direction the other line is in the y
 * direction. Output is a particle trajectory where the particles are fixed
 ...
 * the y direction lines. The field derivatives are stored on the
 * FIELD_DERIV_EVALUATION particle property (REAL, 2 components).
 */
template <typename T> class LineFieldDerivEvaluations {
private:
  int step;
...
public:
   ...
   *  Create new instance.
   *
   * @param field Nektar++ field to sample the derivative of along lines in x
   * and y.
   ...
   */
  LineFieldDerivEvaluations(std::shared_ptr<T> field,
    ...
    ParticleSpec particle_spec{
        ...
        ParticleProp(Sym<REAL>("FIELD_DERIV_EVALUATION"), 2),
        ParticleProp(Sym<INT>("DIRECTION"), 1)};

    NESOASSERT(nx >= 0, "LineFieldDerivEvaluations: bad nx count");
    NESOASSERT(ny >= 0, "LineFieldDerivEvaluations: bad ny count");
    ...
    this->field_evaluate = std::make_shared<FieldEvaluate<T>>(
        field, this->particle_group, this->cell_id_translation, true);
    ...
    this->h5part = std::make_shared<H5Part>(
        ...
        this->particle_group, Sym<REAL>("P"), Sym<INT>("DIRECTION"),
        Sym<REAL>("FIELD_DERIV_EVALUATION"));
  }
  ...
  inline void write(int step_in = -1) {
    ...
    this->field_evaluate->evaluate(Sym<REAL>("FIELD_DERIV_EVALUATION"));
