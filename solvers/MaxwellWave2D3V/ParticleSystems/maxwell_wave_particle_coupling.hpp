...
public:
  ...
  MaxwellWaveParticleCoupling(
    ...
    this->phi_field_evaluate = std::make_shared<FieldEvaluate<T>>(
        this->phi_function, this->charged_particles->particle_group,
    ...
    this->rho_field_project = std::make_shared<FieldProject<T>>(
        this->rho_function, this->charged_particles->particle_group,
    ...
    auto phys_phi = this->phi_function->UpdatePhys();
    auto phys_rho = this->rho_function->UpdatePhys();
  ...
  inline void deposit_charge() {} // TODO
  inline void deposit_current() {} // TODO
